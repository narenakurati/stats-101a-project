...
```{r}
#simran's model using invserse response plot
library(alr3)
...
```{r}
#need to add in Ashwin's code here
```

```{r}
#naren exploring full model with individual changes to variables and subsequent new models
m10 <- lm(happiness_data$Happy ~ factor(happiness_data$Household) + factor(happiness_data$OwnHome) + happiness_data$Instagram + factor(happiness_data$Marital) + happiness_data$Children + happiness_data$Education + factor(happiness_data$JobSat) + happiness_data$Income + factor(happiness_data$Sex) + happiness_data$Age)
summary(m10)

#removing Children, Instagram, Sex, Education
m11 <- lm(happiness_data$Happy ~ factor(happiness_data$Household) + factor(happiness_data$OwnHome) + factor(happiness_data$Marital) + factor(happiness_data$JobSat) + happiness_data$Age+ happiness_data$Income)
summary(m11)
inverse.response.plot(m11,key=TRUE)

#adding in inverse response plot lambda and after removing
m12 <- lm((happiness_data$Happy)^-0.1808482 ~ factor(happiness_data$Household) + factor(happiness_data$OwnHome) + factor(happiness_data$Marital) + factor(happiness_data$JobSat) + happiness_data$Age + log(happiness_data$Income))
summary(m12)

#with ashiwns updates
age_transform <- (happiness_data$Age)^0.226
m13 <- lm((happiness_data$Happy)^-0.1130549 ~ factor(happiness_data$Household) + factor(happiness_data$OwnHome) + factor(happiness_data$Marital) + factor(happiness_data$JobSat) + age_transform + log(happiness_data$Income))
summary(m13)
plot(m13)
```

```{r}
#naren testing with means of categorical variables

```

```{r}
#jeremy commits again
```
