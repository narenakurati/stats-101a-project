...
---
##Introduction
* In this study, we are looking to create a model to predict *Happiness*. We have variables *Household*, *Health*, *OwnHome*, *Instagram*, *Marital*, *Sex*, *Age*, *Children*, *Education*, *JobSat*, *Income*, and *WorkHrs*. By finding a viable model, we could be able to use this model for increasing happiness in a variety of applications including academic settings to social welfare.
...
We start with manual F-tests based on backward selection (removing the least significant variables first each iteration).The following variables must be explored further: *Instagram*, *Children*, *OwnHome*, *Sex*, *Age*, and *Education*.
```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
...
We tried the step-function to select significant variables but couldn't execute the function properly because of varying missing values in each model. So, we started off with manual partial F-tests to compare the reduced and full model and eliminate the insignificant variables,.. We remove all insigificant variables (*Instagram*, *Children*, *Education*, *Sex*). Our $R^2$ and $R^2_{adj}$ values dropped significantly to 0.3088144 and 0.237442 respectively, but we had to do so to avoid overfitting. 
```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
noMarital <- lm(log(Happy) ~ Sex + JobSat.f + OwnHome.f + Household + Education_transformed + Age_transformed + Income_log + Children + Instagram.f); anova(full_model_transform_log_inverse_response, noMarital)$`Pr(>F)` 
#Marital is significant; remaining anova tests present in RMD file
```

```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60), echo = FALSE}
noHousehold <- lm(log(Happy) ~ Sex + JobSat.f + OwnHome.f + Marital.f + Education_transformed + Age_transformed + Income_log + Children + Instagram.f); #anova(full_model_transform_log_inverse_response, noHousehold)$`Pr(>F)`
#Household is significant
noSex <- lm(log(Happy) ~ JobSat.f + OwnHome.f + Household + Marital.f + Education_transformed + Age_transformed + Income_log + Children + Instagram.f); #anova(full_model_transform_log_inverse_response, noSex)$`Pr(>F)`
#Sex is insignificant
noInstagram <- lm(log(Happy) ~ Sex + JobSat.f + OwnHome.f + Household + Marital.f + Education_transformed + Age_transformed + Income_log + Children); #anova(full_model_transform_log_inverse_response, noInstagram)$`Pr(>F)`
#Instagram is insignificant
noChildren <- lm(log(Happy) ~ Sex + JobSat.f + OwnHome.f + Household + Marital.f + Education_transformed + Age_transformed + Income_log + Instagram.f); #anova(full_model_transform_log_inverse_response, noChildren)$`Pr(>F)`
#Children is insignificant
noEducation <- lm(log(Happy) ~ Sex + JobSat.f + OwnHome.f + Household + Marital.f + Age_transformed + Income_log + Children + Instagram.f); #anova(full_model_transform_log_inverse_response, noEducation)$`Pr(>F)`
#Education is insignificant
```

```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
updated_model <- lm(log(Happy) ~ Household.f + OwnHome.f + Marital.f + JobSat.f + Income_log  + Age_transformed)
...
##AIC, AICc, BIC
AIC, AICc, and BIC subsets show a subset of size 1 to be optimal. However, doing so drops our $R^2$ and $R^2_{adj}$ significantly and cuts our number of predictors drastically. We choose not to implement the subset of size 1.
```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60), echo = FALSE}
om1 <- lm(Happy ~ Household.f)
om2 <- lm(Happy ~ Household.f + OwnHome.f)
om3 <- lm(Happy ~ Household.f + OwnHome.f + Marital.f)
om4 <- lm(Happy ~ Household.f + OwnHome.f + Marital.f + JobSat.f)
om5 <- lm(Happy ~ Household.f + OwnHome.f + Marital.f + JobSat.f + Income_log)
om6 <- lm(Happy ~ Household.f + OwnHome.f + Marital.f + JobSat.f + Income_log  + Age_transformed)
...
opmodel <- data.frame(Size=1:6, AIC=AIC, AICc=AICc, BIC=BIC)
opmodel
```
...
full_interaction_terms <- lm(log(Happy) ~ Household.f + OwnHome.f + Marital.f + JobSat.f + Income_log  + Age_transformed)
   
full_interaction_terms <- lm(log(Happy) ~ Household.f + OwnHome.f + Marital.f + JobSat.f + Income_log  + Age_transformed + JobSat.f:Income + JobSat.f:Age_transformed + OwnHome.f:Age_transformed + OwnHome.f:Income + Household.f:Income + Household.f:Age_transformed + Marital.f:Income + Marital.f:Age_transformed)
 
#Deleting insignificant interaction terms
final_model <- lm(log(Happy) ~ JobSat.f + OwnHome.f + Marital.f + Household.f + Income_log + Age_transformed + Household.f:Income)

final_model$r.squared
final_model$adj.r.squared
```


##Leverages, Outliers, and Influential Points
Now we see how many bad leverage points we have. We have 9 bad leverage points. Amongst 186 observations, this is reasonable. We conclude that our final model is accurate.
```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
StanRes1 <- rstandard(final_model); leverage1 <- hatvalues(final_model); cookd1 <- cooks.distance(final_model); p <- 7; n <- 186
...
* Our analysis is limited in that it only uses 6 predictors. Happiness is a human emotion; it is thus complicated and certainly cannot be definitively predicted by 6 factors, or even 12, for that matter. This is part of why $R^2$ and $R^2_{adj}$ values are low in social sciences; human behavior is immensely difficult to predict.
